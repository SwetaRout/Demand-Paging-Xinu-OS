...
//kprintf("INITIALISED\n");
	...
	pd_t *pdentry;
	for(i=0;i < 4;i++){
		bs_map_num=bs_map_num+1;
		get_frm(&frame_num);
//		kprintf("\n frame num init = %d \n",frame_num);
		ptentry = (FRAME0 + frame_num) * NBPG;
		...
		for(j=0;j<1024;j++)
		{
				ptentry->pt_pres = 1;	
  			    	...
  			    	ptentry->pt_dirty = 0;
   		    		...
   		    		ptentry->pt_global = 1;
  			    	ptentry->pt_avail = 0;
  			    	ptentry->pt_base = i*FRAME0 + j;	
	//			kprintf("pt_base = %d",ptentry->pt_base);
	  		    	ptentry++;
		}
	}
...
void initpagedir(int pid)
{
//        kprintf("\n ****** init page dir for pid = %d *******\n",pid);
        int i=0,frame_num=0;
        pd_t *pdentry;

        get_frm(&frame_num);
//        kprintf("FRAME NUMBER = %d\n\n",frame_num);
        pdentry = (FRAME0+ frame_num) * NBPG;
        ...
        for(i=0;i<1024;i++)
        {
                pdentry[i].pd_write = 1;
                if(i<4)
                {
                        pdentry[i].pd_pres=1;
                        pdentry[i].pd_base=FRAME0+i;
	//		kprintf("\n dir base = %d \n",pdentry[i].pd_base);
                }
        }
}
